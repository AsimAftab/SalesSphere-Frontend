...
const PartyPage: React.FC = () => {
  ...
  // ✅ FIX: Removed .id access. Only using ._id as defined in the interface.
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const columns = [
        ...
        { header: 'Phone', key: 'phone', width: 15, style: { alignment: { horizontal: 'left' } } }, 
      ...
      const rows = partiesData.map((party: any, index: number) => {
        ...
        if (party.image) {
            rowData.image = { text: party.image, hyperlink: party.image, tooltip: 'Click to open image' };
        } else {
            rowData.image = 'N/A';
        }
        return rowData;
