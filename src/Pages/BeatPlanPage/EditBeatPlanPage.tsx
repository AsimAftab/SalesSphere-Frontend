...
  getAvailableParties,
  updateBeatPlan,
  type UpdateBeatPlanPayload as ServiceUpdateBeatPlanPayload,
  type SimpleParty,
...
  const dropdownRef = useRef<HTMLDivElement>(null);
  ...
  const { data: employeesData, isLoading: isLoadingEmployees, error: employeesError } = useQuery<SimpleSalesperson[], Error, Employee[]>({
    ...
    const { data: allPartiesData, isLoading: isLoadingAllParties, error: partiesError } = useQuery<SimpleParty[], Error, Shop[]>({
  ...
  const { data: planData, isLoading: isLoadingPlanDetails, error: planError } = useQuery<BeatPlanType, Error>({
        queryKey: BEAT_PLAN_KEYS.detail(planId || ''),
        queryFn: () => getBeatPlanById(planId!),
        enabled: !!planId, 
    });
    
  const updateMutation = useMutation<any, Error, { id: string; updateData: ServiceUpdateBeatPlanPayload }>({
        ...
        onError: (error) => {
            setFetchError(`Failed to update plan: ${error.message}`);
        }
    });


    useEffect(() => {
        if (planData && allPartiesData && employeesData) {
            
            setSelectedEmployee(planData.employees[0]?._id || ''); 
            setPlanName(planData.name);
            const assignedDate = new Date(planData.schedule.startDate);
            setSelectedDate(isNaN(assignedDate.getTime()) ? null : assignedDate);

            const assignedPartyIds = new Set(planData.parties.map(p => p._id));
            const allShopsMapped = allPartiesData; 
            setAssignedRoute(allShopsMapped.filter(s => assignedPartyIds.has(s.id)));
            setAvailableShops(allShopsMapped.filter(s => !assignedPartyIds.has(s.id)));
        }
    }, [planData, allPartiesData, employeesData]); 
...
        (shop.zone ?? '').toLowerCase().includes(term)
    );
...
    if (assignedRoute.length === 0 && errors.shops) { setErrors(prev => ({ ...prev, shops: undefined })); }
  };
...
    if (!planName.trim()) { newErrors.planName = 'Please enter a beat plan name.'; }
    if (assignedRoute.length === 0) { newErrors.shops = 'Please assign at least one shop.'; }
    return newErrors;
  };
...
    const payload: ServiceUpdateBeatPlanPayload = {
...
 
  const selectedEmployeeName = employeesData?.find(emp => emp.id === selectedEmployee)?.name || 'Choose employee...';


    const isLoadingInitialData = isLoadingPlanDetails || isLoadingAllParties || isLoadingEmployees;
    const isError = planError || employeesError || partiesError;
...
        <div className="p-6 text-center text-red-600">
            {fetchError || (planError as Error)?.message || (employeesError as Error)?.message || (partiesError as Error)?.message || 'Unknown error loading data.'}
        </div>
      </Sidebar>
    );
  }
...
                <div className="flex justify-center items-center h-full text-gray-500">
               <Loader2 className="h-6 w-6 animate-spin mr-2" /> Loading Parties...
                </div>
...
               Assigned Route ({assignedRoute.length} Parties)
            </h2>
           {errors.shops && (
