...
interface Day {
  day: number;
  weekday: string;
  isWeeklyOff: boolean; // 1. CHANGED from 'isWeekend'
}
      ...
      <View style={styles.table}>
        <View style={styles.tableHeader} fixed>
          ...
          <View style={styles.daysContainer}>
            {days.map(day => (
              // 2. CHANGED 'day.isWeekend' to 'day.isWeeklyOff'
              <View key={day.day} style={[styles.dayHeaderCell, day.isWeeklyOff ? styles.weekendHeader : {}]}>
                <Text>{day.day}</Text>
          ...
          <View style={styles.tableRow} key={employee.name} wrap={false}>
            ...
            <View style={styles.daysContainer}>
              {employee.attendanceString.split('').map((status, idx) => (
                // 3. CHANGED 'days[idx].isWeekend' to 'days[idx].isWeeklyOff'
                <Text key={idx} style={[styles.dayCell, days[idx].isWeeklyOff ? styles.weekendCell : {}, getStatusStyle(status)]}>{status}</Text>
              ))}
            </View>
