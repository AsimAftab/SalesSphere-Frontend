...
// Import the Mapper to centralize domain logic
...
export const MiscWorkMobileList: React.FC<Props> = ({ 
  data, 
  selectedIds, 
  onToggle, 
  onViewImage, 
    ...
    {data.map((work) => (
      <div
        ...
        className={`p-4 rounded-xl border shadow-sm relative overflow-hidden transition-colors ${
          selectedIds.includes(work._id) ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200'
        }`}
      >
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-center gap-3">
            ...
            {/* Fallback logic using centralized initials and secondary branding */}
            <div className="h-12 w-12 rounded-full bg-secondary text-white font-bold flex items-center justify-center border border-secondary shrink-0 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-3 mb-4">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-600">
            ...
            <span>
              Assigned by: <span className="font-bold">{work.assignedBy?.name || "Admin"}</span>
            </span>
          </div>
        </div>
