...
  const [selectedStatus, setSelectedStatus] = useState<string>(initialStatusFilter); 
  const [selectedDate, setSelectedDate] = useState<string>(initialDateFilter); // <-- ADD THIS LINE
...
        const orderDate = new Date(order.dateTime);
        ...
        // Month filter: 'all' or matches the month
        const monthMatch = 
          selectedMonth === 'all' || 
          months[orderDate.getMonth()] === selectedMonth;

        // Status filter: 'all' or matches the status
        const statusMatch =
          selectedStatus === 'all' ||
          order.status.toLowerCase() === selectedStatus.toLowerCase();

        // Search filter: matches party or invoice
        const searchMatch =
          (order.partyName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
          (order.invoiceNumber || '').toLowerCase().includes(searchTerm.toLowerCase());
        
        // ADDED Date filter: 'all' or is today
        const dateMatch = 
          selectedDate === 'all' || 
          (selectedDate === 'today' && isToday(order.dateTime));

        return monthMatch && statusMatch && searchMatch && dateMatch; // Added dateMatch
      })
      .sort(
        (a, b) =>
          new Date(b.dateTime).getTime() - new Date(a.dateTime).getTime()
      );
  }, [data, searchTerm, selectedMonth, selectedStatus, selectedDate]); // <-- Added selectedDate
...
                  className="border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring focus:ring-blue-200"
                >
                  <option value="all">All Months</option> {/* <-- ADDED THIS OPTION */}
                  {months.map(month => (
