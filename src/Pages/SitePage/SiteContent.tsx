...
// Kept Skeleton imports as they are used in SiteContentSkeleton component below
  ...
  const handleAddSite = async (data: NewEntityData) => {
    try {
      const newSiteData: NewSiteData = {
        ...
        siteInterest: data.prospectInterest, 
      };
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden pb-6">
             ...
             {totalPages > 1 && (
              <div className="flex-shrink-0 flex items-center justify-between mt-6 text-sm text-gray-600 pt-4 border-t border-gray-200">
                <p>Showing {startIndex + 1} - {Math.min(endIndex, filteredSite.length)} of {filteredSite.length}</p>
