...
// Icons are still needed for the cards, just not the buttons
...
    const [selectedParty, setSelectedParty] = useState<FullPartyDetailsData['party'] | null>(null);
...
    };
    ...
    const handleDeleteClick = (partyToDelete: FullPartyDetailsData['party']) => {
    setSelectedParty(partyToDelete); // Use the parameter name
    ...
    if (selectedParty) {
        try {
            // Use the imported deletePartyDetails and the correct ID property
            await deletePartyDetails(selectedParty.id);
            // Use navigate to redirect after successful deletion
            navigate('/parties');
        } catch (error) {
            console.error('Error deleting party:', error);
        } finally {
            // Ensure modal closes and state resets even if error occurs
...
                    {/* Delete Button (styled as outline) */}
                    <Button
                        variant="outline"
...
                            <p className="text-md font-medium text-gray-900">{party.phone}</p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-500 mb-1">PAN/VAT Number</p>
                            <p className="text-md font-medium text-gray-900">{party.panVat}</p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-500 mb-1">Email Address</p>
                            <p className="text-md font-medium text-gray-900">{party.email}</p>
                        </div>
                        
                        <div>
...
                        </h3>
                    </div>
                    {/* Map container now uses flex-1 to grow and fill h-full parent */}
...
            />
            <ConfirmationModal
                ...
                message={`Are you sure you want to delete "${selectedParty?.companyName}"?`}
                onConfirm={confirmDelete}
                onCancel={() => { setDeleteModalOpen(false); setSelectedParty(null); }}
            />
        </div>
    );
};
