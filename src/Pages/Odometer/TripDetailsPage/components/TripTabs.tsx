...
const TripTabs: React.FC<TripTabsProps> = ({ trips, activeTripId, onTabChange }) => {
    return (
        <div className="w-fit mb-6 flex overflow-x-auto gap-2 scrollbar-hide">
            {trips.map((trip) => {
                ...
                return (
                    <Button
                        ...
                        className={`
                            min-w-[100px] rounded-full text-sm font-semibold transition-colors duration-200 hover:transform-none
                            ${isActive
