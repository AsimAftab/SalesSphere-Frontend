...
const TripTabs: React.FC<TripTabsProps> = ({ trips, activeTripId, onTabChange }) => {
    return (
        <div className="w-fit bg-white p-2 rounded-xl mb-6 flex overflow-x-auto gap-2 border border-gray-100 no-scrollbar">
            {trips.map((trip) => {
                ...
                return (
                    <Button
                        ...
                        className={`
                            min-w-[100px] rounded-full text-sm font-semibold transition-all duration-200
                            ${isActive
