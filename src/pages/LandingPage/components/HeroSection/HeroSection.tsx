...
import { ArrowRight, CheckCircle2, Store, ShoppingCart, Clock, IndianRupee, Menu } from 'lucide-react';
  ...
  >
    ...
    >
      ...
      <div className="flex bg-white">
        {/* Sidebar - full height */}
        <div className="hidden sm:flex border-r border-gray-100 p-2.5 w-[130px] xl:w-[145px] bg-white flex-shrink-0 flex-col">
          ...
          <div className="space-y-px">
            {HERO_NAV_ITEMS.map((item) => (
              ...
              >
                ...
                />
                <span className={`text-[7px] font-semibold truncate ${item.active ? 'text-white' : 'text-gray-700'}`}>
                  {item.name}
                </span>
              </div>
            ))}
          ...
          <div className="border-t border-gray-100 pt-1.5 space-y-px">
            <div className="flex items-center gap-1.5 px-1.5 py-[3.5px]">
              ...
              <span className="text-[7px] font-semibold text-gray-700">Settings</span>
            </div>
            <div className="flex items-center gap-1.5 px-1.5 py-[3.5px]">
              <img src={logoutIcon} alt="" className="w-3 h-3" />
              <span className="text-[7px] font-semibold text-gray-700">Logout</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center justify-between px-3 py-1.5 bg-white border-b border-gray-100">
            <div className="flex items-center gap-2 flex-wrap">
              <Menu className="sm:hidden w-2.5 h-2.5 text-gray-500 mr-0.5" />
              <span className="text-[7.5px] font-bold text-gray-800">SalesSphere Demo Corp</span>
              <div className="h-3 w-px bg-gray-200" />
              <div className="hidden sm:flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-emerald-50 border border-emerald-100">
                ...
                <span className="text-[6px] font-semibold text-emerald-700"><span className="font-bold">365</span> days remaining</span>
              </div>
            </div>
            <div className="flex items-center gap-1.5">
              ...
              <div>
                ...
                <p className="text-[5.5px] text-gray-500">Admin</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 bg-[#f8f9fb] p-3 min-w-0">
            {/* Greeting */}
            <div className="mb-2">
              <p className="text-[9px] font-bold text-gray-800">Good Evening, <span className="text-blue-700">Admin!</span></p>
              <p className="text-[5px] text-gray-500">Saturday, February 7, 2026</p>
            </div>

            {/* 5 Stat cards */}
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-1.5 mb-2">
              {[
                { label: 'TOTAL NO. OF PARTIES', value: '156', iconBg: 'bg-blue-100', iconColor: 'text-blue-700', Icon: Store },
                { label: "TODAY'S TOTAL PARTIES", value: '12', iconBg: 'bg-blue-100', iconColor: 'text-blue-700', Icon: Store },
                { label: "TODAY'S TOTAL ORDERS", value: '38', iconBg: 'bg-purple-100', iconColor: 'text-purple-700', Icon: ShoppingCart },
                { label: 'TOTAL PENDING ORDERS', value: '7', iconBg: 'bg-orange-100', iconColor: 'text-orange-700', Icon: Clock },
                { label: "TODAY'S ORDER VALUE", value: 'Rs 19,600', iconBg: 'bg-green-100', iconColor: 'text-green-700', Icon: IndianRupee },
              ].map((stat) => (
                <div key={stat.label} className="bg-white rounded-lg p-1.5 border border-gray-100 flex items-center justify-between">
                  <div>
                    <p className="text-[4px] text-gray-500 uppercase tracking-wider mb-0.5 leading-tight">{stat.label}</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                  ...
                  {[
                    ...
                    <div key={m.name} className="flex items-center gap-1.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[3.5px] text-gray-500">{m.role}</p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="text-[5px] font-bold text-green-700">{m.amount}</p>
                        <p className="text-[3.5px] text-gray-500">{m.orders} orders</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-white rounded-lg p-2 border border-gray-100">
                ...
                <div className="flex items-baseline justify-between mb-1">
                  <span className="text-[11px] font-bold text-green-600">83%</span>
                  <span className="text-[4.5px] text-gray-500">12 Team Members</span>
                </div>
                  ...
                  {ATTENDANCE_ITEMS.map((item) => (
                    <div key={item.label} className="flex items-center justify-between">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[4.5px] text-gray-600">{item.label}</span>
                      </div>
                      <span className="text-[4.5px] font-semibold text-gray-700">{item.count}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg p-2 border border-gray-100">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="flex items-center gap-0.5">
                    ...
                    <span className="text-[4.5px] font-semibold text-blue-700">5 Active</span>
                  </div>
                </div>
                <div>
                  {[
                    ...
                    <div key={emp.name}>
                      <div className="flex items-center gap-1.5 py-1">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[3.5px] text-gray-500">{emp.role}</p>
                        </div>
                        <span className="text-[3.5px] text-gray-500 flex-shrink-0">{emp.beat}</span>
                      </div>
                      {i < 2 && <div className="border-t border-gray-50" />}
                    </div>
                  ))}
                  <p className="text-[4.5px] text-blue-700 font-semibold mt-1 text-center">View All Live Employees →</p>
                </div>
              </div>
              {/* Sales Trend */}
              <div className="bg-white rounded-lg p-2 border border-gray-100 col-span-1 sm:col-span-3">
                <div className="flex items-center justify-between mb-1.5">
                  <p className="text-[6.5px] font-bold text-gray-700">Sales Trend (Last 7 Days)</p>
                  <span className="text-[5px] text-green-700 font-semibold bg-green-50 px-1.5 py-0.5 rounded-full">+23.5%</span>
                </div>
                <svg viewBox="0 0 340 80" className="w-full h-[65px]">
                  ...
                  {[0, 5000, 10000, 15000, 20000].map((val, i) => {
                    ...
                    return (
                      <g key={val}>
                        <text x="28" y={y + 1} textAnchor="end" className="fill-gray-500" style={{ fontSize: '4px' }}>{`₹${val / 1000}k`}</text>
                  ...
                  {['Feb 1', 'Feb 2', 'Feb 3', 'Feb 4', 'Feb 5', 'Feb 6', 'Feb 7'].map((d, i) => {
                    ...
                    return (
                      <g key={d}>
                        ...
                        <text x={x + barWidth / 2} y="75" textAnchor="middle" className="fill-gray-500" style={{ fontSize: '4px' }}>{d}</text>
                      </g>
                    );
                  })}
                </svg>
...
const HeroBadge = memo<{ text?: string }>(({ text }) => {
  ...
  return (
    <motion.div
      ...
      className="inline-flex items-center gap-2 px-3 sm:px-5 py-1.5 sm:py-2.5 mb-3 sm:mb-6 lg:mb-8 bg-white/10 border border-white/20 rounded-full backdrop-blur-sm"
    >
      <motion.div
        initial={{ scale: 0 }}
        animate={{
...
const HeroHighlights = memo<HeroHighlightsProps>(({ highlights }) => {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="mt-4 sm:mt-8 lg:mt-10 flex flex-col sm:flex-row sm:flex-wrap gap-2.5 sm:gap-4 items-center justify-center lg:justify-start px-4 sm:px-0"
    >
...
const HeroCTAGroup = memo<HeroCTAGroupProps>(({ primaryCta, secondaryCta }) => (
  <motion.div
    ...
    initial="hidden"
    animate="visible"
    className="mt-6 sm:mt-10 lg:mt-12 flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center lg:justify-start px-4 sm:px-0"
  >
    <motion.button
      ...
      >
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 lg:flex-1 lg:flex lg:items-center">
          <div className="w-full py-8 pb-12 sm:py-12 sm:pb-16 lg:py-0 lg:pb-0">
            <div className="grid lg:grid-cols-2 xl:grid-cols-[1fr_1.3fr] 2xl:grid-cols-[1fr_1.4fr] gap-6 lg:gap-8 xl:gap-10 items-center">
              {/* Left Column - Content */}
              <motion.div
