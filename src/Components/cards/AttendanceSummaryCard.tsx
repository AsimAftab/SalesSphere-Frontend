...
const AttendanceSummaryCard = ({ data }) => (
  <div className="bg-white p-6 rounded-lg shadow-sm h-full">
    ...
    <div className="space-y-3">
      ...
      <div className="pt-2">
        <div className="flex justify-between mb-1"><span className="text-sm font-medium text-gray-700">Attendance Rate</span><span className="text-sm font-medium text-gray-700">{data.rate}%</span></div>
        <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-blue-600 h-2 rounded-full" style={{ width: `${data.rate}%` }}></div></div>
      </div>
    </div>
  </div>
);
