...
// --- 2. Update Header to accept 'user' prop ---
interface HeaderProps {
    onMenuClick: () => void;
    user: Employee | null; // Accept the user object (or null while loading)
}

const Header: React.FC<HeaderProps> = ({ onMenuClick, user }) => {
    
    // --- 3. Create fallback values ---
...
  const [sidebarOpen, setSidebarOpen] = useState(false);
  ...
  // --- 6. Add state for user ---
  const [user, setUser] = useState<Employee | null>(null);

  // --- 7. Fetch user data on mount ---
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        // Optional: handle logout or error display
      }
    };
    fetchUser();
