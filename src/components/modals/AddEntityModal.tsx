...
interface AddEntityModalProps {
  ...
  // ✅ Updated: Accept full category data objects
...
const AddEntityModal: React.FC<AddEntityModalProps> = ({
  ...
  const resetCurrentInterestEntry = () => {
    ...
    setTechnicianName('');
    setTechnicianPhone('');
    setAvailableBrands([]); // ✅ Clear available brands on reset
  ...
  // ✅ 1. Reset form ONLY when modal opens
  useEffect(() => {
    if (isOpen) {
      handleClose(false); 
    }
  ...
  // ✅ 2. Update Categories list whenever data changes (without resetting form)
  useEffect(() => {
    if (categoriesData) {
  ...
  // --- INTEREST HANDLERS (UPDATED LOGIC) ---
  ...
  const handleCategorySelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // ✅ Update Available Brands based on selected category from categoriesData
    if (val && val !== 'ADD_NEW') {
  ...
  const handleAddCustomBrand = () => {
    ...
    // Temporarily add to local available list if it's a new brand
  ...
  const handleAddInterestEntry = () => {
    ...
    // If a totally new category was typed, temporarily add it to available list
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const dataToSave: NewEntityData = {
      ...
      prospectInterest: interests.length > 0 ? interests : undefined
    };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-[9999]">
      <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto flex flex-col">
        ...
        <div className="flex justify-between items-center p-5 border-b border-gray-100 sticky top-0 bg-white z-10">
          ...
          <button onClick={() => handleClose(true)} className="p-1 rounded-full text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          {/* --- PROSPECT INTEREST SECTION --- */}
          {showInterestSection && (
            <div className="space-y-4 bg-gray-50 p-4 rounded-xl border border-gray-200 mt-6">
              ...
              {!isInterestCollapsed && (
                <div className="space-y-6 pt-2">
                        ...
                        <div key={idx} className="flex justify-between items-start bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              {item.brands.map(b => (
                                <span key={b} className="text-[10px] bg-blue-50 text-secondary px-2 py-0.5 rounded-full border border-blue-100">{b}</span>
                              ))}
                            </div>
                  ...
                  <div className="bg-white p-4 rounded-lg border border-blue-200 shadow-sm">
                    <h4 className="text-sm font-bold text-secondary mb-3 uppercase tracking-wide">Add Interest Details:</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <input 
                              ...
                              className={`mt-0 ${inputClass('newCategory')} pr-8`} 
                      ...
                      {/* ✅ Brand Multi-Select - Updates dynamically */}
                      <div className="md:col-span-2">
                        ...
                        {currentBrands.length > 0 && (
                          <div className="mb-2 flex flex-wrap gap-2">
                            {currentBrands.map(brand => (
                              <span key={brand} className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-blue-100 text-secondary">
                                {brand}
                                <button type="button" onClick={() => removeBrandChip(brand)} className="ml-1 text-secondary hover:text-secondary"><XMarkIcon className="h-3 w-3"/></button>
                              </span>
                            ))}
                          </div>
                        )}
                        ...
                        <div className="relative">
                          <select 
                            ...
                            className={`${inputClass('brands')} appearance-none pr-10 w-full`}
                          >
                          ...
                          <div className="flex gap-2">
                            <div className="relative flex-1">
                              <input 
                                ...
                                className={`${inputClass('brands')} pr-8`}
                            ...
                            <Button 
                              ...
                              className="px-3 py-1.5 text-sm bg-secondary text-white rounded-lg hover:bg-secondary"
                            >
                              Add
                            </Button>
                            <button 
                              type="button" 
                              onClick={handleCancelCustomBrand} 
                      ...
                      {/* Site Specifics (Forced New Line) */}
                      {entityType === 'Site' && (
                        <div className="md:col-span-2 mt-2">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              <label className="text-xs font-semibold text-gray-500 uppercase mb-1">Technician Name</label>
                              <input type="text" value={technicianName} onChange={(e) => setTechnicianName(e.target.value)} className={`mt-0 ${inputClass('technicianName')}`} />
                              {errors.technicianName && <p className="text-red-500 text-xs mt-1">{errors.technicianName}</p>}
                            </div>
                            <div>
                              <label className="text-xs font-semibold text-gray-500 uppercase mb-1">Technician Phone</label>
                              <input type="tel" value={technicianPhone} onChange={(e) => setTechnicianPhone(e.target.value.replace(/[^0-9]/g, '').slice(0,10))} maxLength={10} className={`mt-0 ${inputClass('technicianPhone')}`} />
                              {errors.technicianPhone && <p className="text-red-500 text-xs mt-1">{errors.technicianPhone}</p>}
                            </div>
                          </div>
                        </div>
                      )}
                    ...
                    <div className="mt-4 flex justify-end">
                      <Button type="button" className="py-1 px-3 text-sm" onClick={handleAddInterestEntry}>
          ...
          {/* Description */}
          <div>
            <div className="border-b pb-2 mb-2 mt-6"><h3 className="text-lg font-semibold flex items-center gap-2 text-gray-800"><DocumentTextIcon className="w-5 h-5 text-blue-600"/> Additional Info</h3></div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Description <span className="text-red-500">*</span></label>
            <textarea name="description" rows={3} value={formData.description} onChange={handleChange} className={inputClass('description')} placeholder="Add notes..." disabled={isSaving}/>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
            ...
            <Button type="submit" variant="primary" disabled={isSaving}>
              {isSaving ? 'Saving...' : `Create ${entityType}`}
            </Button>
          </div>
        </form>
      </div>
