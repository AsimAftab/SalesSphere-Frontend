...
const AddEntityModal: React.FC<AddEntityModalProps> = ({
  ...
  const handleAddTechnician = () => {
    if (!techNameInput.trim()) {
      toast.error('Technician name is required');
      return;
    }
    if (!techPhoneInput.trim() || techPhoneInput.length !== 10) {
  ...
  const handleAddInterestEntry = () => {
    ...
    if (!catSelectValue) {
      ...
      if (!catInputValue.trim()) {
        ...
        finalCategory = catInputValue.trim();
      }
    } else {
  ...
  const handleRemoveInterestItem = (index: number) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSite && isAddingNewSubOrg && formData.subOrgName && onAddSubOrg) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-[9999]">
      <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto flex flex-col">
        ...
        <button onClick={() => handleClose(true)} className="p-1 rounded-full text-gray-400 hover:bg-red-100 hover:text-red-600 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
            ...
            {/* Row 1: Name & Owner */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{nameLabel} <span className="text-red-500">*</span></label>
              <input type="text" name="name" value={formData.name} onChange={handleChange} className={inputClass('name')} placeholder={namePlaceholder} disabled={isSaving} />
              {errors.name && <p className="mt-1 text-sm text-red-500">{errors.name}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{ownerLabel} <span className="text-red-500">*</span></label>
              <input type="text" name="ownerName" value={formData.ownerName} onChange={handleChange} className={inputClass('ownerName')} placeholder={ownerPlaceholder} disabled={isSaving} />
            ...
            <div className="md:col-span-2 pt-4 pb-2 mt-4 border-t border-b border-gray-200">
               <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2"><PhoneIcon className="w-5 h-5 text-blue-600" /> Contact Details</h3>
            </div>
            <div><label className="block text-sm font-medium text-gray-700 mb-2">Phone Number <span className="text-red-500">*</span></label><input type="tel" name="phone" value={formData.phone} onChange={handleChange} maxLength={10} className={inputClass('phone')} placeholder="Enter 10-digit phone" disabled={isSaving} />{errors.phone && <p className="mt-1 text-sm text-red-500">{errors.phone}</p>}</div>
            <div><label className="block text-sm font-medium text-gray-700 mb-2">Email Address</label><input type="email" name="email" value={formData.email} onChange={handleChange} className={inputClass('email')} placeholder="Enter email" disabled={isSaving} />{errors.email && <p className="mt-1 text-sm text-red-500">{errors.email}</p>}</div>
            ...
            <div className="md:col-span-2 mt-4">
              ...
              <div className="h-72 rounded-lg" style={{ pointerEvents: 'auto' }}><LocationMap position={mapPosition} onLocationChange={handleMapSync} onAddressGeocoded={handleAddressSync} /></div>
            </div>
            <div className="md:col-span-2"><label className="block text-sm font-medium text-gray-700 mb-2">Address <span className="text-red-500">*</span></label><p className={`${readOnlyFieldClass} min-h-[78px]`}>{formData.address || 'Auto-filled from map'}</p>{errors.address && <p className="mt-1 text-sm text-red-500">{errors.address}</p>}</div>
            <div><label className=" text-sm font-medium text-gray-700 mb-2 flex items-center gap-1"><GlobeAltIcon className="w-4 h-4 text-gray-500"/> Latitude <span className="text-red-500">*</span></label><p className={readOnlyFieldClass}>{formData.latitude}</p></div>
            <div><label className=" text-sm font-medium text-gray-700 mb-2 flex items-center gap-1"><GlobeAltIcon className="w-4 h-4 text-gray-500"/> Longitude <span className="text-red-500">*</span></label><p className={readOnlyFieldClass}>{formData.longitude}</p></div>
              ...
              <div className="md:col-span-2 mt-6">
                <div className="space-y-4 bg-gray-50 p-4 rounded-xl border border-gray-200">
                  <div className="flex justify-between items-center cursor-pointer" onClick={() => setIsInterestCollapsed(!isInterestCollapsed)}>
                    <h3 className="text-lg font-semibold flex items-center gap-2 text-gray-800"><BriefcaseIcon className="w-5 h-5 text-secondary"/> {entityType} Interest Details <span className="bg-blue-100 text-secondary text-xs font-bold px-2 py-0.5 rounded-full">{interests.length}</span></h3>
                    <button type="button" className="text-gray-500 hover:text-secondary">{isInterestCollapsed ? <ChevronDownIcon className="h-5 w-5"/> : <PlusIcon className="h-5 w-5 rotate-45"/>}</button>
                  </div>
                          ...
                          {interests.map((item, idx) => (
                            <div key={idx} className="flex justify-between items-start bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                              <div>
                                ...
                                <div className="flex flex-wrap gap-1 mt-1">{item.brands.map(b => <span key={b} className="text-sm bg-blue-50 text-secondary px-2 py-0.5 rounded-full border border-blue-100">{b}</span>)}</div>
                              ...
                              <button type="button" onClick={() => handleRemoveInterestItem(idx)} className="text-red-400 hover:text-red-600 p-1"><TrashIcon className="h-4 w-4"/></button>
                            </div>
                          ))}
                        </div>
                      )}
                          ...
                          {isAddingNewCategory && (
                            <div>
                              ...
                              <div className="relative">
                                ...
                                {catInputValue && <button type="button" onClick={handleClearCategoryInput} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"><XMarkIcon className="h-4 w-4" /></button>}
                              </div>
                            ...
                            {currentBrands.length > 0 && (
                              <div className="mb-2 flex flex-wrap gap-2">
                                {currentBrands.map(brand => (
                                  <span key={brand} className="inline-flex items-center px-2 py-1 rounded-md text-sm font-medium bg-blue-100 text-secondary">{brand}<button type="button" onClick={() => removeBrandChip(brand)} className="ml-1 text-secondary hover:text-secondary"><XMarkIcon className="h-3 w-3"/></button></span>
                                ))}
                              </div>
                            )}
                            <div className="relative">
                          ...
                          {isAddingNewBrand && (
                            <div className="md:col-span-2">
                              ...
                              <div className="flex gap-2">
                                ...
                                <button type="button" onClick={handleCancelCustomBrand} className="p-2 text-gray-500 hover:text-gray-700"><XMarkIcon className="h-5 w-5" /></button>
                              </div>
                            </div>
                          )}
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
            <Button type="button" variant="ghost" onClick={() => handleClose(true)} disabled={isSaving}>Cancel</Button>
            <Button type="submit" variant="primary" disabled={isSaving}>{isSaving ? 'Saving...' : `Create ${entityType}`}</Button>
          </div>
        </form>
      </div>
    </div>
